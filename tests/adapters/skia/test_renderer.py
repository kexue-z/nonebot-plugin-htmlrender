...
class _FailingWorker:
    async def run_sync(
        ...
        *args: object,
        ...
        del function, args
        raise _NativeExplosion("native surface failed")
...
async def test_native_failure_is_translated_to_stable_backend_error() -> None:
    ...
    assert "native surface failed" in str(raised.value)


async def test_observation_identifies_skia_backend(
    ...
    assert operation_observer.operations == [
        (
            ...
            {"render.backend": "skia", "render.format": "png"},
            "success",
        )
    ]
