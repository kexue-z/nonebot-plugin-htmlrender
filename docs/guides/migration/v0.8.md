...
0.7 与 0.8 开发分支中曾存在的过渡 Provider 接口不构成兼容契约。第三方Provider 必须适配 0.8.0a1 起公开的类型化 settings、`ProviderDependencies`、`EngineBindings`、`ProviderResources`、`ResourceStrategy` 与 provider-local
