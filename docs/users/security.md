...
HTML 渲染会连接网络、读取资源、执行模板或运行浏览器。Provider 不替调用方
建立业务级信任边界。
...
Jinja autoescape 不能替代模板源码信任。模板源码、filters 与 extensions 只能
来自受控代码；用户输入只能作为 `variables` 值。
...
- 不向不可信调用方暴露 raw Playwright Page；
...
不要记录 HTML、URL、路径、模板变量、header、字体名、digest 或 bytes。只使用
低基数 operation、Provider identity、status 和聚合 cache 统计。observer
异常必须与业务结果隔离。
