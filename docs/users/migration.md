...
- 远程渲染（远程 Playwright / 远程浏览器）：[远程 Playwright 与 Filehost](remote-playwright.md)
...
2. 再移除生命周期旧入口，如 `startup_htmlrender`、`shutdown_htmlrender`
3. 最后处理只在少量路径出现的兼容模块导入，如 `browser.py`、`data_source.py`

## 迁移建议节奏

1. 先在 `.env` 中加入 `RENDER_BACKEND` 与 `RENDER_STARTUP_MODE`，确认插件可正常启动。
2. 把使用最多的旧 API（通常是 `md_to_pic` / `template_to_pic`）替换为新 API，验证渲染产物一致。
3. 处理日志中的弃用警告，逐个移除剩余旧 API 调用。
4. 视需求接入资源解析、远程渲染、可观测性扩展。

## 升级后验收清单

- [ ] `render_text` 或 `render_markdown` 本地渲染成功
- [ ] 模板目录渲染成功，`template_name`、`base_url` 行为符合预期
- [ ] 如使用远程浏览器，WS 或 CDP 连接正常
- [ ] 如模板里含本地静态资源，资源解析与 filehost 行为符合预期
